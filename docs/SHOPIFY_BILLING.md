...
- [ ] **Usage record creation** — the `cappedAmount` and `usageTerms` are configured on the Shopify subscription, but the backend does not currently call Shopify's `appUsageRecordCreate` mutation to report actual overage consumption. Without this, Shopify will not bill merchants for overages beyond the recurring charge. Implement the `appUsageRecordCreate` GraphQL call in `BillingEntitlementService` when `consumed_count > included_limit`.
