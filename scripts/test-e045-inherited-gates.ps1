...
try {
    $containerId = (& docker run --detach --rm `
        --label akeed.e045.inherited-gates=true `
        --publish '127.0.0.1::5432' `
        --env POSTGRES_USER=e01_test `
        --env POSTGRES_PASSWORD=e01-synthetic-only `
        --env POSTGRES_DB=akeed_e01_test `
        $postgresImage).Trim()
    if ($LASTEXITCODE -ne 0 -or $containerId -notmatch '^[a-f0-9]{64}$') {
        throw 'Could not create the disposable inherited-gate PostgreSQL container.'
    }
    ...
    if ($containerId -match '^[a-f0-9]{64}$') {
        ...
        if ($LASTEXITCODE -ne 0) {
            $testExitCode = 1
            Write-Warning 'Disposable inherited-gate PostgreSQL cleanup failed.'
        }
    }
    Pop-Location
}
